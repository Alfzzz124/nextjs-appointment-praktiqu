...
import { getUpcomingSessions } from '@/services/billing/dashboard.service';
...
export const GET = withAuth(async (req: NextRequest, ctx) => kcHandle(async () => {
  ...
  return kcOk(await getUpcomingSessions(parsed.data as any, dashboardScopeFor(kc)), 'Upcoming sessions');
}));
