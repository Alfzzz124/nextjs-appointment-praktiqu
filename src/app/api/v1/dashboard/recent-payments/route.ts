...
import { getRecentPayments } from '@/services/billing/dashboard.service';
...
export const GET = withAuth(async (req: NextRequest, ctx) => kcHandle(async () => {
  ...
  return kcOk(await getRecentPayments(parsed.data as any, dashboardScopeFor(kc)), 'Recent payments');
}));
