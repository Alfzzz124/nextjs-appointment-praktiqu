...
import { getTopProfessionals } from '@/services/billing/dashboard.service';
...
export const GET = withAuth(async (req: NextRequest, ctx) => kcHandle(async () => {
  ...
  return kcOk(await getTopProfessionals(parsed.data as any, dashboardScopeFor(kc)), 'Top professionals');
}));
