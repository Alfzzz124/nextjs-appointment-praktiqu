...
export const POST = withAuth(async (req: NextRequest, ctx) => kcHandle(async () => {
  ...
  if (rows.length === 0) return kcFail('No rows to import', 400);
  if (rows.length > MAX_ROWS) return kcFail(`Too many rows: ${rows.length} (max ${MAX_ROWS})`, 400);

  const kc = await resolveKcActor(actor);
  const out = await runImport(entity as ImportEntity, rows, { conflictStrategy: conflictStrategy as ConflictStrategy, dryRun }, kc);
