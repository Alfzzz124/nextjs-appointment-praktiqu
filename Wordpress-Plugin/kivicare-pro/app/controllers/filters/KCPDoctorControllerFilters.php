...
class KCPDoctorControllerFilters
{
    /**
     * Override the parent method to add custom functionality
     */
    private static ?KCPDoctorControllerFilters $instance = null;

    public function __construct()
    {
        // Add custom form data to doctor create/update endpoint args
        add_filter('kc_doctor_create_endpoint_args', [$this, 'addCustomFormArgs'], 10, 1);
        add_filter('kc_doctor_update_endpoint_args', [$this, 'addCustomFormArgs'], 10, 1);

        // Save custom form data on doctor create/update
        add_action('kc_doctor_save', [$this, 'saveCustomFormData'], 10, 2);
        add_action('kc_doctor_update', [$this, 'saveCustomFormData'], 10, 2);
    }

    public static function get_instance(): KCPDoctorControllerFilters|null {
    ...
    /**
     * Add custom form data argument to doctor create/update endpoints
     *
    ...
    {
        $args['customForm'] = [
            'description' => 'Custom form data for doctor',
            'type' => 'object',
    ...
    /**
     * Save custom form data for doctor
     *
     * @param array $doctorData The doctor data
     * @param WP_REST_Request $request The request object
     */
    public function saveCustomFormData($doctorData, WP_REST_Request $request)
    {
        ...
        foreach ($customFormData as $formId => $formFields) {
            ...
            // Check if record already exists for this form and doctor
            $existingRecord = KCCustomFormData::query()
                ->where('form_id', (int) $formId)
                ->where('module_id', (int) $doctorData['id'])
                ->first();
            ...
            if ($existingRecord) {
                ...
                $customFormEntry->moduleId = (int) $doctorData['id'];
