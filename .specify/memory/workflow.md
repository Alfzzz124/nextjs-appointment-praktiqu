...
- **Durable Project Memory (`docs/memory/`)**:
  ...
  *   `docs/memory/architecture/` (detailed boundaries and integrations)
  *   `docs/memory/decisions/` (technical and implementation decisions)
  *   `docs/memory/bugs/` (recurring bugs and mitigation patterns)
  *   `docs/memory/worklog/` (sequential ledger of durable changes)
...
- **Isolation**: Ensure new changes do not violate trust boundaries or architectural layers defined in `docs/memory/architecture/ARCHITECTURE.md`.
...
*This is a living document. Update when process improves, but preserve existing project-specific governance rules.*
